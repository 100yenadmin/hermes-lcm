...
| `lcm_recall` | Search the agent's entire memory across ALL conversations and all time by meaning. Runs three arms over the whole local database — full-text raw messages, embedded summary KNN, and verbatim chunk KNN (all cross-session, no filter) — fuses them with RRF, dedupes chunk hits against FTS by `store_id`, and applies a soft prior `final_score = rank_score × (1 + scope_bias × is_current_conversation) × recency_boost(half_life=30d, floor 0.5)`. `scope_bias` (0..1, default 0.5) and recency are ranking BOOSTS, never filters. `include` selects `all`/`summaries`/`verbatim`. The omitted or explicit `detail='snippets'` path remains byte-compatible. Opt into `detail='answer_ready'` for bounded per-session diversity and exact-source windows carrying `exact_ref`, its required `evidence_identity`, immutable source provenance, observation/write-time metadata, and optional occurrence time. |
| `lcm_compute` | Execute a dependency-free deterministic date, count, compatible-unit sum, difference, or ordering operation over exact cited message spans. Every operand must include the `evidence_identity` emitted with its ref. Values, units, labels, keys, and dates must be explicit in the source span. Source deletion, content/provenance drift, overlapping spans, explicit as-of violations, mixed temporal expressions, ambiguity, mixed units, and incomplete evidence fail closed to `next_path='evidence_only'`. |
...
| `lcm_load_session` | Load one ordered raw-message transcript page for an explicit `session_id`. This is not search: it returns raw rows in `store_id` order, bounded by `limit`, with per-message content bounded by `max_content_chars`, and continues with `after_store_id` from `next_cursor`. `include_exact_ref=true` adds a ref and its `evidence_identity` for each returned slice; omission preserves default bytes. |
| `lcm_describe` | Inspect the current-session DAG or preview an `externalized_ref` without loading full content. |
| `lcm_expand` | Recover source messages, child summaries, or externalized payloads with pagination. Use `store_id` to fetch a single raw message regardless of session. In that mode, `include_exact_ref=true` adds a ref whose span follows the page's `content_offset` plus its `evidence_identity`. |
