...
`store_id` fix, and the benchmark evidence trail (`bench/`, F20–F34).

## Measured results (all paired, pinned, reproducible — provenance blocks in bench/)
- LongMemEval-V1: ⟨TBD⟩/500 on the consolidated base (sanity-slice-validated against the banked 444/500 within
  the measured noise floor; retrieval byte-identical on 96% of questions vs the prior base).
...
- 389× scaling probe: retrieval p50 267 ms at ~200k messages pre-fix (sub-linear, faster than file-scan at
  every rung); post-fix curve: ⟨TBD F34⟩.
